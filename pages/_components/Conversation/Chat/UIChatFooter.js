...
const ChatInput = ({
  ...
  const { imagesdata, bufferdata } = useSelector(
    state => state.form.formData.uploadChatImage
  )
  ...
  return (
    ...
    >
      <Stack sx={{ backgroundColor: '#ECECEC', borderRadius: '25px' }}>
        ...
        <input
          style={{
            border: darkMode ? '' : '1px solid #DADADA',
            paddingLeft: '50px',
            paddingTop: '12px',
            paddingBottom: '12px',
            borderRadius: '25px',
            backgroundColor: darkMode ? '#464646' : '#FFF'
          }}
...
const ChatFooter = ({ formData, updateFormData }) => {
              ...
              axios
                ...
                .then(() => {
                  const chatContent2 = {
                    ...
                    message: message || '',
                    fileName: imagesdata?.key,
                    type: imagesdata?.contentType.startsWith('video')
                      ? 'video'
                      : 'image'
                  }
  ...
  return (
    <Box
      p={2}
      sx={{ width: '100%', backgroundColor: darkMode ? '#2B2D30' : '#FFF' }}
    >

...
function UserProfileLikeBtn ({ userdata, userId, TheUsername }) {
  ...
  const HandleShortlist = () => {
    if (userdata?.userShortListDetails) {
      dispatch(
        RemoveShortlist(
          userdata?.userShortListDetails?.id ||
            userdata?.userShortListDetails?._id
        )
      )
    } else {
      dispatch(addToShortlist(userdata?._id || userdata?.id))
    ...
    setTimeout(() => {
      ...
      dispatch(fetchUserById(userdata?._id || userdata?.id))
    }, 900)
  ...
  useEffect(() => {
    if (data?.results) {
      const liked = data.results.some(
        item => item.likedUserId === userId && item.isLike
      )
      const disliked = data.results.some(
        item => item.likedUserId === userId && !item.isLike
      )
      ...
      <div className='flex justify-center space-x-4'>
        ...
        <div>
          <button
            ...
            className={
              userdata?.userShortListDetails
                ? 'h-[40px] w-[63px] rounded-[22px] grid place-items-center border-[1px] border-[#FFBC47] bg-[#FFBC47]'
                : 'shortlist-button h-[40px] w-[63px] rounded-[22px] grid place-items-center border-[1px] border-[#FFBC47]'
            }
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              ...
              className={
                userdata?.userShortListDetails
                  ? 'the-shortlist-icon-active'
                  : 'the-shortlist-icon'
              }
              fill='none'
            >
              <path
                d='M9.26316 14.1448L14.9878 17.6L13.4686 11.088L18.5263 6.70653L11.8661 6.14147L9.26316 0L6.66021 6.14147L0 6.70653L5.05768 11.088L3.53853 17.6L9.26316 14.1448Z'
                // fill='#FFBC47'
              />
            </svg>
          </button>
...
export default UserProfileLikeBtn

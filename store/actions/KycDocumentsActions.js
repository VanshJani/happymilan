...
export const UploadKycDocument = (documentname, requestdata) => {
    console.log("🚀 ~ UploadKycDocument ~ documentname:", documentname)
    console.log("🚀 ~ UploadKycDocument ~ requestdata:", requestdata)
    return async (dispatch) => {
        ...
        try {
            ...
            console.log("Doc URL : ", response.data.data?.docUrl)
            ...
            if (docUrl && status === "Success") {
                ...
                let config = {
                    method: 'post',
                    maxBodyLength: Infinity,
                    url: 'https://stag.mntech.website/api/v1/user/kyc/',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${authToken}`
                    },
            ...
            console.log("S3 Upload Response:", s3Response.data); // Log S3 response
...
export const CheckDocumentStatus = (request) => {
    return async (dispatch) => {
        ...
        let config = {
            method: 'get',
            maxBodyLength: Infinity,
            url: `https://stag.mntech.website/api/v1/user/kyc/by-user/${userID}`,
            headers: {
                'Content-Type': 'application/json',
